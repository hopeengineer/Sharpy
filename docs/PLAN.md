...
| Decision | Locked by |
|---|---|
| **whisper-turbo + parakeet produce the transcript; their agreement is the per-word confidence. Apple `SpeechAnalyzer` is live preview only.** | On the real narration (§2.7) Apple had 18 disagreement sites and ≈9 adjudicated errors including one that inverts meaning; whisper-turbo and parakeet each err only where they disagree with each other. Together they cost 13 s per 88 s of speech. Apple keeps the instant-preview role: 94× realtime, 20 MB RSS, model in the OS. |
...
| **Parakeet is the second engine, never the only one.** | Best clean WER (0.57 %) and the only engine that got the meaning-critical "didn't" right on real speech, but ≈5 other errors and sub-word tokens that must be merged into words; 5.5 GB peak on 203 s, so it runs at ingest, never resident. |
