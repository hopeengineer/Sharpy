...
func writeMessage(_ payload: [String: Any]) {
...
func encodeID(_ id: JSONValue?) -> Any {
    switch id {
...
func respond(id: JSONValue?, result: [String: Any]) {
...
final class Session {
    ...
    init() throws { store = try IndexStore() }

    var document: Document? { log?.head }

    func requireDocument() throws -> Document {
        guard let d = log?.head else { throw SessionError.noMediaOpen }
        return d
    }
    func requireTranscript() throws -> Transcript {
        guard let t = transcript else { throw SessionError.noTranscript }
        return t
    }
    var frameRate: FrameRate { log?.head.timeline.frameRate ?? .r30 }
}
...
func runTool(_ name: String, _ args: JSONValue?, _ session: Session) -> [String: Any] {
